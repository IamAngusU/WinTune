...
$script:WtaTexts = @{
    en = @{
        ...
        RecycleBinUnavailable='Windows Recycle Bin is unavailable in this session.'
        RecycleBinPictures='DELETED PICTURES IN RECYCLE BIN'
        RecycleBinNoPictures='No supported picture files were found in Recycle Bin.'
        RecycleBinInventorySaved='Local inventory saved: {0}'
        RecycleBinInventoryEmpty='No Recycle Bin pictures found. Local inventory: {0}'
        RecycleBinChoose='Enter picture numbers to restore, separated by commas. Press Enter to cancel'
        RecycleBinNoSelection='No pictures were selected for restoration.'
        RecycleBinInvalidSelection='No valid picture selection was entered.'
        RecycleBinRestoreNotice='{0} picture(s) will be restored to their original location. Existing files are not overwritten automatically.'
        RecycleBinRestoreConfirm='Type RESTORE exactly to restore the selected pictures'
        RecycleBinRestoreDeclined='Picture restoration was declined.'
        RecycleBinRestoreUnavailable='Windows did not provide a restore command for this picture.'
        RecycleBinRestoreResult='Restore requested for {0} picture(s); {1} could not be requested. Check the original location and the local inventory.'
        PermanentDeletionGuidance='If Recycle Bin was emptied, stop saving files to that drive. WinTune will not write recovery data to it. Recover to a different drive with a trusted recovery tool, such as Windows File Recovery, as soon as possible.'
    }
    de = @{
        ...
        RecycleBinUnavailable='Der Windows-Papierkorb ist in dieser Sitzung nicht verfügbar.'
        RecycleBinPictures='GELÖSCHTE BILDER IM PAPIERKORB'
        RecycleBinNoPictures='Im Papierkorb wurden keine unterstützten Bilddateien gefunden.'
        RecycleBinInventorySaved='Lokale Liste gespeichert: {0}'
        RecycleBinInventoryEmpty='Keine Bilder im Papierkorb gefunden. Lokale Liste: {0}'
        RecycleBinChoose='Bildnummern zum Wiederherstellen kommagetrennt eingeben. Enter bricht ab'
        RecycleBinNoSelection='Keine Bilder zur Wiederherstellung ausgewählt.'
        RecycleBinInvalidSelection='Keine gültige Bildauswahl eingegeben.'
        RecycleBinRestoreNotice='{0} Bild(er) werden an ihrem ursprünglichen Ort wiederhergestellt. Vorhandene Dateien werden nicht automatisch überschrieben.'
        RecycleBinRestoreConfirm='RESTORE exakt eingeben, um die ausgewählten Bilder wiederherzustellen'
        RecycleBinRestoreDeclined='Die Bildwiederherstellung wurde abgelehnt.'
        RecycleBinRestoreUnavailable='Windows hat für dieses Bild keinen Wiederherstellungsbefehl bereitgestellt.'
        RecycleBinRestoreResult='Wiederherstellung für {0} Bild(er) angefordert; für {1} konnte sie nicht angefordert werden. Ursprünglichen Speicherort und lokale Liste prüfen.'
        PermanentDeletionGuidance='Wenn der Papierkorb geleert wurde, keine weiteren Dateien auf diesem Laufwerk speichern. WinTune schreibt keine Wiederherstellungsdaten darauf. Möglichst bald mit einem vertrauenswürdigen Recovery-Tool wie Windows File Recovery auf ein anderes Laufwerk wiederherstellen.'
    }
}
